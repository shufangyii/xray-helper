...
install_dependencies() {
    ...
    apt install -y nginx curl socat certbot python3-certbot-nginx ufw
    if [ $? -ne 0 ]; then
...
show_results() {
    ...
    echo
    echo -e "你可以将以上参数填入v2rayN, Shadowrocket等客户端进行连接。"
    echo -e "访问 https://${DOMAIN} 应该会看到Nginx的欢迎页面。"
    echo -e "${GREEN}================================================================${PLAIN}"
    echo
}
...
show_cloudflare_instructions() {
    ...
    echo
    echo -e "${GREEN}2. 添加你的域名:${PLAIN} 将你的域名 ${GREEN}${DOMAIN}${PLAIN} 添加到Cloudflare。"
    echo
    ...
    echo -e "   - 确保记录类型为 ${BLUE}A${PLAIN}，名称为你的子域名 (如vpn)，内容为你的VPS IP地址。"
